...
public class LargestSmallest 
{
    public static void main( String[] args )
    {
        ...
        int largest = 0; // Variable to compare with user's input and determine largest
        ...
        // make first integer as a smallest
        smallest = number1;
        
        System.out.print( "Please, Enter second integer: "); // prompt for input
        number2 = input.nextInt(); // read second integer
        
        // compare second integer with largest and smallest and update vales of them
        if ( smallest >= number2 )
            smallest = number2;
        if ( largest < number2 ) // largest = 0, so don't use = sign
            largest = number2; // if second integer is larger than first put it in largest
